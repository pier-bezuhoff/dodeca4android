...
// rough version
class RoughPrimitiveCircles(cs: List<CircleFigure>, private val paint: Paint) : CircleGroup {
    private var size: Int = cs.size
    ...
    private var oldXs: FloatArray = xs // old_s are used for draw and as oldCircles in update
    ...
    override val figures: List<CircleFigure>
        get() = (0 until size).map { i ->
            val (color, fill, rule, borderColor) = attrs[i]
            CircleFigure(
                xs[i].toDouble(), ys[i].toDouble(), rs[i].toDouble(),
                color, fill, rule, borderColor
            )
        }
    ...
    override fun get(i: Int): CircleFigure {
        val (color, fill, rule, borderColor) = attrs[i]
        return CircleFigure(
            oldXs[i].toDouble(), oldYs[i].toDouble(), oldRs[i].toDouble(),
            color, fill, rule, borderColor
        )
    }
